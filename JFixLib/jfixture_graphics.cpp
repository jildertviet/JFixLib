...
void jFixtureGraphics::init() { jFixture::init(); }

void jFixtureGraphics::update() { jFixture::update(); }
...
void jFixtureGraphics::sync(int eventID) {
  ...
  if (e) {
    e->syncTime = (uint64_t)(esp_timer_get_time() / 1000);
  }
}

// ---- Binary-format handlers (called from legacy path, kept until full
// ProtoBuf migration) ----
...
void jFixtureGraphics::addEvent(const uint8_t *data, int data_len) {
  ...
  case 0x01: { // JEvent_Perlin
    ...
    if (data_len >= 1 + (int)(sizeof(float) * 5))
      memcpy(&p->size, data + 1 + sizeof(int) + sizeof(float) * 2,
             sizeof(float) * 2);
    if (data_len >= 1 + (int)(sizeof(float) * 9))
      memcpy(&p->rgba, data + 1 + sizeof(int) + sizeof(float) * 4,
             sizeof(float) * 4);
    ...
    if (data_len >= 1 + 1 + (int)(sizeof(float) * 10))
      memcpy(&p->noiseScale, data + 1 + 1 + sizeof(int) + sizeof(float) * 9,
             sizeof(float));
    if (data_len >= 1 + 1 + (int)(sizeof(float) * 11))
      memcpy(&p->noiseTimeScale,
             data + 1 + 1 + sizeof(int) + sizeof(float) * 10, sizeof(float));
    if (data_len >= 1 + 1 + (int)(sizeof(float) * 12))
      memcpy(&p->horizontalPixelOffset,
             data + 1 + 1 + sizeof(int) + sizeof(float) * 11, sizeof(float));
  ...
  case 0x02: { // JRect
    ...
    if (data_len >= 1 + (int)(sizeof(float) * 5))
      memcpy(&r->size, data + 1 + sizeof(int) + sizeof(float) * 2,
             sizeof(float) * 2);
    if (data_len >= 1 + (int)(sizeof(float) * 9))
      memcpy(&r->rgba, data + 1 + sizeof(int) + sizeof(float) * 4,
             sizeof(float) * 4);
    if (data_len >= 1 + (int)(sizeof(float) * 9) + 1)
  ...
  case 0x03: { // JOsc
    ...
    if (data_len >= 1 + (int)(sizeof(float) * 5))
      memcpy(&j->size, data + 1 + sizeof(int) + sizeof(float) * 2,
             sizeof(float) * 2);
    if (data_len >= 1 + (int)(sizeof(float) * 9))
      memcpy(&j->rgba, data + 1 + sizeof(int) + sizeof(float) * 4,
             sizeof(float) * 4);
    if (data_len >= 1 + (int)(sizeof(float) * 10))
      memcpy(&j->frequency, data + 1 + sizeof(int) + sizeof(float) * 9,
             sizeof(float));
    if (data_len >= 1 + (int)(sizeof(float) * 11))
      memcpy(&j->offset, data + 1 + sizeof(int) + sizeof(float) * 10,
             sizeof(float));
    if (data_len >= 1 + (int)(sizeof(float) * 12))
      memcpy(&j->range, data + 1 + sizeof(int) + sizeof(float) * 11,
             sizeof(float));
    if (data_len >= 1 + 1 + (int)(sizeof(float) * 12)) {
      float powVal;
      memcpy(&powVal, data + 1 + sizeof(int) + sizeof(float) * 12,
             sizeof(float));
...
void jFixtureGraphics::addEnv(const uint8_t *data, int data_len) {
  // Binary layout: [float a, float s, float r, float b, char bKill, char
  // varName, int eventID]
  if (data_len < (int)(sizeof(float) * 4 + 2 + sizeof(int)))
    return;
  ...
  switch (varName) {
  case 'b':
    e->addEnv('b', &e->brightness, (uint16_t)a, (uint16_t)s, (uint16_t)r, b,
              0.0f, bKill);
    break;
  case 'x':
    e->addEnv('x', &e->loc[0], (uint16_t)a, (uint16_t)s, (uint16_t)r, b,
              e->loc[0], bKill);
    break;
  case 'y':
    e->addEnv('y', &e->loc[1], (uint16_t)a, (uint16_t)s, (uint16_t)r, b,
              e->loc[1], bKill);
    break;
  case 'w':
    e->addEnv('w', &e->size[0], (uint16_t)a, (uint16_t)s, (uint16_t)r, b,
              e->size[0], bKill);
    break;
  case 'h':
    e->addEnv('h', &e->size[1], (uint16_t)a, (uint16_t)s, (uint16_t)r, b,
              e->size[1], bKill);
    break;
  }
  e->updateEnvelopes();
...
void jFixtureGraphics::setVal(const uint8_t *data, int data_len) {
  ...
  // For varName='c': [int eventID, char 'c', float r, float g, float b, float
  // a]
  if (data_len < (int)(sizeof(int) + 1 + sizeof(float)))
    return;
  ...
  if (varName == 'c' &&
      data_len >= (int)(sizeof(int) + 1 + sizeof(float) * 4)) {
...
void jFixtureGraphics::setCustomArg(const uint8_t *data, int data_len) {
  ...
  if (data_len < (int)(sizeof(int) + sizeof(float) * 2))
    return;
