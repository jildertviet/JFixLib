...
#if !defined(JFIX_EMULATION)
#include "esp_sleep.h"
#endif

static const char *TAG = "jFixtureAddr";

jFixtureAddr::jFixtureAddr() { this->writeRGBPtr = &jFixtureAddr::writeRGB; }
...
void jFixtureAddr::writeRGBHard(int id, float r, float g, float b,
                                uint8_t channel, floatColor **leds) {
  if (!leds || !leds[channel])
    return;
  leds[channel][id].r = r;
  leds[channel][id].g = g;
...
void jFixtureAddr::update() {
  Parser::getInstance().drainCommandQueue();
  jFixtureGraphics::update();

  if (bIdle) {
    allBlack(true);
#ifndef JFIX_EMULATION
    if (nextLightSleepUs > 0) {
      uint64_t us = nextLightSleepUs;
      nextLightSleepUs = 0;
  ...
  uint32_t now = (uint64_t)(esp_timer_get_time() / 1000);
  if (now - lastUpdatedLive < 20)
    return;
  lastUpdatedLive = now;
  ...
  for (int i = 0; i < MAX_EVENTS; i++) {
    if (events[i]) {
      events[i]->update();
      if (events[i]->bActive) {
        events[i]->draw(ledBuffer, numLedsPerString, numStrings,
                        horizontalPixelDistance);
      } else {
        delete events[i];
        events[i] = nullptr;
      }
...
void jFixtureAddr::blink(uint8_t num, uint16_t dur, uint16_t delayTime,
                         uint8_t channel) {
