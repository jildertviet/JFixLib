...
const uint8_t EspnowHandler::broadcast_mac[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
...
esp_err_t EspnowHandler::init() {
    // Ensure WiFi is initialized. If connectWiFi was called, it's already done.
    // If not, we might need a basic init here.
    // In jFixture, connectWiFi is called before espnow init.
...
void EspnowHandler::receive_cb(const esp_now_recv_info_t *recv_info, const uint8_t *data, int len) {
