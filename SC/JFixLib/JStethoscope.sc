JStethoscope {
	...
	*new {
		...
		zoom = 1.0, rate = \audio, view, bufnum, bAddSliders=true;
		var bus;
		...
		^super.new.initStethoscope( server, view, bus, bufsize, 1024 * zoom.asFloat.reciprocal, bAddSliders);
	}

	initStethoscope { arg server_, parent, bus_, bufsize_, cycle_, bAddSliders_;
		var singleBus;

		server = server_;
		bAddSliders = bAddSliders_;
		synth = JBusScopeSynth(server);
		...
		makeGui = { arg parent;
			...
			if(bAddSliders == true, {
			view.layout.add(yZoomSlider.maxWidth_(15), 1,1)
