...
JEspnowDevice::JEspnowDevice() {}

void JEspnowDevice::setup(String networkName) {
  JFixture::setup(networkName);
  initEspnow("JV_");
  pingOffsetSeed = random(2000);
}

String JEspnowDevice::randomPw() {
  String r = "01234567";
  for (int i = 0; i < 8; i++) {
    r[i] = random(255);
  }
...
bool JEspnowDevice::checkAddressed(const uint8_t *data) {
...
void JEspnowDevice::receive(const uint8_t *mac_addr, const uint8_t *data,
                            int data_len) {
  char msgType = data[0];
...
void JEspnowDevice::setBootMode(char *data) {
...
void JEspnowDevice::saveMsg(const uint8_t *data, const short data_len) {
...
void JEspnowDevice::parseMultiple(const uint8_t *data, const uint8_t data_len) {
  char numMsg = data[0];
  ...
  for (int i = 0; i < MAX_MSG; i++) { // Reverse order
    if (msgBuffer[i].len) {
      ...
      case 0x37:
        ...
        break;
      case 0x38:
        setParameterBus((msgBuffer[i].data) + 6 + 1, msgBuffer[i].len - 6 - 1);
        break;
      case 0x39:
        setParameterBusN((msgBuffer[i].data) + 6 + 1, msgBuffer[i].len - 6 - 1);
        break;
      }
...
void JEspnowDevice::initEspnow(String networkName) {
...
void JEspnowDevice::addPeer(uint8_t *addr) {
...
void JEspnowDevice::sendPing(bool bOverride) {
  if (otaMode != IDLE)
    return;
  if (!bEspnowEnabled)
    return;
